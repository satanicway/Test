...
def initial_spawn(board):
    """Place 2 rifts, 2 monsters and 2 quests on random free nodes."""
    ...
    for loc in picks[4:6]:
        board[loc].append(Spot('Q'))

def spawn_new_quest(verbose=False):
    """Spawn a quest at a random free node."""
...
def spawn_end_of_round(rnd, verbose=False):
    """Spawn rifts and monsters at the end of the given round."""
    if rnd >= TOTAL_ROUNDS:
        return

    if rnd <= 3:
        rifts = 1 if random.random() < 0.75 else 0
    elif rnd <= 6:
        rifts = 1 + (random.random() < 0.5)
    elif rnd <= 8:
        rifts = 2 + (random.random() < 0.5)
    else:
        rifts = 0

    free = [n for n in ALL if n != CENTRE and not board.get(n)]
    random.shuffle(free)
    for loc in free[:rifts]:
        board[loc].append(Spot('R'))
        if verbose:
            print(f"    Spawned Rift at {loc}")

    rift_locs = [loc for loc, pile in board.items() if any(s.t == 'R' for s in pile)]
    for loc in rift_locs:
        if random.random() < 0.75:
            board[loc].append(Spot('M'))
            if verbose:
                print(f"    Spawned Monster at {loc}")
